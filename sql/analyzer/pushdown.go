...
func transformPushdownFilters(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, tableAliases TableAliases, indexes indexLookupsByTable) (sql.Node, error) {
	applyFilteredTables := func(n *plan.Filter, filters *filterSet) (sql.Node, bool, error) {
		c := plan.TransformContext{n, nil, -1, sql.Schema{}}
		return plan.TransformUpCtxHelper(c, filterPushdownChildSelector, func(c plan.TransformContext) (sql.Node, bool, error) {
			switch node := c.Node.(type) {
			case *plan.Filter:
				n, mod, err := removePushedDownPredicates(ctx, a, node, filters)
				if err != nil {
					return nil, false, err
				}
				n, modFix, err := FixFieldIndexesForExpressions(ctx, a, n, scope)
				if err != nil {
					return nil, false, err
				}
				return n, mod || modFix, nil
			case *plan.IndexedTableAccess:
				lookup, ok := indexes[node.Name()]
				if !ok || lookup.expr == nil {
					return node, false, nil
				}
				handled, mod, err := pushdownFiltersToIndex(ctx, a, node, lookup, tableAliases)
				if err != nil {
					return nil, false, err
				}
			...
			case *plan.TableAlias, *plan.ResolvedTable, *plan.ValueDerivedTable:
				n, mod, err := pushdownFiltersToTable(ctx, a, node.(NameableNode), scope, filters, tableAliases)
				if err != nil {
					return nil, false, err
				}
				n, modFix, err := FixFieldIndexesForExpressions(ctx, a, n, scope)
	...
	pushdownAboveTables := func(n sql.Node, filters *filterSet) (sql.Node, bool, error) {
		c := plan.TransformContext{n, nil, -1, sql.Schema{}}
		return plan.TransformUpCtxHelper(c, filterPushdownAboveTablesChildSelector, func(c plan.TransformContext) (sql.Node, bool, error) {
			switch node := c.Node.(type) {
			case *plan.Filter:
				n, mod, err := removePushedDownPredicates(ctx, a, node, filters)
				if err != nil {
				...
				return n, false, nil
			case *plan.TableAlias, *plan.ResolvedTable, *plan.IndexedTableAccess, *plan.ValueDerivedTable:
				table, mod, err := pushdownFiltersToAboveTable(ctx, a, node.(NameableNode), scope, filters)
				if err != nil {
					return nil, false, err
				}
				if mod {
	...
	return plan.TransformUp(n, func(node sql.Node) (sql.Node, bool, error) {
		...
		case *plan.Filter:
			...
			node, modRem, err := applyFilteredTables(n, filters)
			if err != nil {
				return nil, false, err
			}

			// Then move filter predicates directly above their respective tables in joins
			node, modPush, err := pushdownAboveTables(node, filters)
			if err != nil {
				return nil, false, err
			}
			return node, modRem || modPush, nil
		case *plan.Window:
...
func transformPushdownSubqueryAliasFilters(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, tableAliases TableAliases) (sql.Node, error) {
	...
	transformFilterNode := func(n *plan.Filter) (sql.Node, bool, error) {
		c := plan.TransformContext{n, nil, -1, sql.Schema{}}
		return plan.TransformUpCtxHelper(c, filterPushdownChildSelector, func(c plan.TransformContext) (sql.Node, bool, error) {
			switch node := c.Node.(type) {
			case *plan.Filter:
...
func convertFiltersToIndexedAccess(
	...
	node, err := plan.TransformUpCtx(n, childSelector, func(c plan.TransformContext) (sql.Node, bool, error) {
		switch node := c.Node.(type) {
		...
		case *plan.TableAlias:
			table, mod, err := pushdownIndexesToTable(a, node, indexes)
			if err != nil {
				return nil, false, err
			}
			if !mod {
			...
			n, _, err := plan.TransformUpHelper(table, func(n sql.Node) (sql.Node, bool, error) {
				ita, ok := n.(*plan.IndexedTableAccess)
				if !ok {
					return n, false, nil
				}

				newExprs, mod, err := FixFieldIndexesOnExpressions(ctx, scope, a, table.Schema(), ita.Expressions()...)
				if err != nil {
					return nil, false, err
				}
				if mod {
		...
		case *plan.ResolvedTable:
			table, modTab, err := pushdownIndexesToTable(a, node, indexes)
			if err != nil {
				return nil, false, err
			}
...
func pushdownFiltersToIndex(ctx *sql.Context, a *Analyzer, idxTable *plan.IndexedTableAccess, lookup *indexLookup, tableAliases TableAliases) (handled []sql.Expression, mod bool, err error) {
...
func pushdownFiltersToTable(
	...
	case *plan.ResolvedTable, *plan.TableAlias, *plan.IndexedTableAccess, *plan.ValueDerivedTable:
		...
		return t, mod, err
	default:
		return nil, false, ErrInvalidNodeType.New("pushdownFiltersToTable", tableNode)
	}
}
...
func pushdownFiltersToAboveTable(
	...
	case *plan.ResolvedTable, *plan.TableAlias, *plan.IndexedTableAccess, *plan.ValueDerivedTable:
		...
		if pushedDownFilterExpression != nil {
			return plan.NewFilter(pushedDownFilterExpression, node), true, nil
		}

		return node, false, nil
	default:
		return nil, false, ErrInvalidNodeType.New("pushdownFiltersToAboveTable", tableNode)
	}
}
...
// analysis phase.
func pushdownFiltersUnderSubqueryAlias(ctx *sql.Context, a *Analyzer, sa *plan.SubqueryAlias, filters *filterSet) (sql.Node, bool, error) {
	...
	for i, h := range handled {
		expressionsForChild[i], err = expression.TransformUp(h, func(e sql.Expression) (sql.Expression, bool, error) {
			if gt, ok := e.(*expression.GetField); ok {
				col := childSchema[gt.Index()]
				return gt.WithTable(col.Source).WithName(col.Name), true, nil
			}
			return e, false, nil
		})
	}
...
func pushdownIndexesToTable(a *Analyzer, tableNode NameableNode, indexes map[string]*indexLookup) (sql.Node, bool, error) {
	return plan.TransformUpHelper(tableNode, func(n sql.Node) (sql.Node, bool, error) {
		switch n := n.(type) {
		case *plan.ResolvedTable:
			table := getTable(tableNode)
			if table == nil {
			...
			if _, ok := table.(sql.IndexAddressableTable); ok {
				...
				if ok {
					...
					return plan.NewStaticIndexedTableAccess(n, indexLookup.lookup, indexLookup.indexes[0], indexLookup.fields), true, nil
				}
			}
		}
		return n, false, nil
...
func pushdownProjectionsToTable(
	...
	case *plan.ResolvedTable, *plan.TableAlias, *plan.IndexedTableAccess:
		...
		return node, true, nil
	default:
		return nil, false, ErrInvalidNodeType.New("pushdown", tableNode)
	}
}
...
func transformPushdownProjections(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	...
	node, err := plan.TransformUpCtx(n, selector, func(c plan.TransformContext) (sql.Node, bool, error) {
		...
		if nameable != nil {
			table, mod, err := pushdownProjectionsToTable(a, nameable, fieldsByTable, usedFieldsByTable)
			if err != nil {
				return nil, false, err
			}
			if mod {
...
func removePushedDownPredicates(ctx *sql.Context, a *Analyzer, node *plan.Filter, filters *filterSet) (sql.Node, bool, error) {
	...
	return plan.NewFilter(expression.JoinAnd(unhandled...), node.Child), true, nil
}
