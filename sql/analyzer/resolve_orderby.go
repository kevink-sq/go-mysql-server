...
func pushSortDown(sort *plan.Sort) (sql.Node, bool, error) {
	switch child := sort.Child.(type) {
	case *plan.Project:
		return plan.NewProject(
			child.Projections,
	...
	default:
		...
		if len(children) == 1 {
			newChild, mod, err := pushSortDown(plan.NewSort(sort.SortFields, children[0]))
			if err != nil {
				return nil, false, err
			}
			if mod {
...
func resolveOrderByLiterals(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	return plan.TransformUp(n, func(n sql.Node) (sql.Node, bool, error) {
		...
		if mod {
			return plan.NewSort(fields, sort.Child), true, nil
		}
		return sort, false, nil
	})
}
