...
func qualifyExpression(e sql.Expression, symbols availableNames) (sql.Expression, bool, error) {
	switch col := e.(type) {
	case column:
		if col.Resolved() {
		...
		if col.Table() != "" {
			...
			if !tableFound {
				...
				return nil, false, sql.ErrTableNotFound.New(col.Table() + similar)
			}
		...
		for _, level := range nestingLevels {
			...
			default:
				return nil, false, sql.ErrAmbiguousColumnName.New(col.Name(), strings.Join(tablesForColumn, ", "))
			}
		}
	...
	case *expression.Star:
		...
		if col.Table != "" {
			...
			if !tableFound {
				return nil, false, sql.ErrTableNotFound.New(col.Table)
			}
		}
		return col, false, nil
	default:
		...
		return expression.TransformUpHelper(e, func(e sql.Expression) (sql.Expression, bool, error) {
			if _, ok := e.(*expression.Star); ok {
				return expression.NewStar(), true, nil
			}
			return e, false, nil
		})
	}
...
func resolveColumns(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	...
	return plan.TransformUp(n, func(n sql.Node) (sql.Node, bool, error) {
		...
		return plan.TransformExpressionsWithNode(n, func(n sql.Node, e sql.Expression) (sql.Expression, bool, error) {
			uc, ok := e.(column)
			if !ok || e.Resolved() {
				return e, false, nil
			}
...
func resolveColumnExpression(a *Analyzer, n sql.Node, e column, columns map[tableCol]indexedCol) (sql.Expression, bool, error) {
	...
	if !ok {
		...
		case *expression.UnresolvedColumn:
			...
			return &deferredColumn{uc}, true, nil
		default:
			if table != "" {
				return nil, false, sql.ErrTableColumnNotFound.New(e.Table(), e.Name())
			}
			...
			if stringContains(aliasesInNode, name) {
				return nil, false, sql.ErrMisusedAlias.New(name)
			}

			return nil, false, sql.ErrColumnNotFound.New(e.Name())
		}
	}
...
func pushdownGroupByAliases(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	...
	return plan.TransformUp(n, func(n sql.Node) (sql.Node, bool, error) {
		...
		if ok && len(replacedAliases) > 0 {
			newExprs, mod := replaceExpressionsWithAliases(ex.Expressions(), replacedAliases)
			if mod {
		...
		if len(renames) > 0 {
			for i, expr := range newSelectedExprs {
				var err error
				newSelectedExprs[i], err = expression.TransformUp(expr, func(e sql.Expression) (sql.Expression, bool, error) {
					...
					if ok {
						...
						if to, ok := renames[col.Name()]; ok && aliases[col.Name()] != i {
							return expression.NewUnresolvedColumn(to), true, nil
						}
					}
