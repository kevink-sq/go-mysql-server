		...
		case *plan.SubqueryAlias:
			...
			if len(n.Columns) > 0 {
				...
				if schemaLen != len(n.Columns) {
					return nil, false, sql.ErrColumnCountMismatch.New()
				}
			}
	...
	return plan.TransformUp(n, func(n sql.Node) (sql.Node, bool, error) {
		...
		case *plan.TableAlias:
			if sa, isSA := n.Children()[0].(*plan.SubqueryAlias); isSA {
				return sa.WithName(n.Name()), true, nil
			}
			if ta, isTA := n.Children()[0].(*plan.TableAlias); isTA {
				return ta.WithName(n.Name()), true, nil
			}
			return n, false, nil
		default:
			return n, false, nil
...
func resolveSubqueryExpressions(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	return plan.TransformExpressionsUpWithNode(n, func(n sql.Node, e sql.Expression) (sql.Expression, bool, error) {
		...
		if err != nil {
			...
			if ErrValidationResolved.Is(err) || sql.ErrTableColumnNotFound.Is(err) || sql.ErrColumnNotFound.Is(err) {
				...
				return s.WithQuery(analyzed), true, nil
			}
			return nil, false, err
		}

		return s.WithQuery(StripPassthroughNodes(analyzed)), true, nil
	})
}
...
func cacheSubqueryResults(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	...
	return plan.TransformExpressionsUpWithNode(n, func(n sql.Node, e sql.Expression) (sql.Expression, bool, error) {
		...
		if cacheable {
			return s.WithCachedResults(), true, nil
		}
...
func cacheSubqueryAlisesInJoins(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	n, err := plan.TransformUpCtx(n, nil, func(c plan.TransformContext) (sql.Node, bool, error) {
		...
		if isJoin || isIndexedJoin {
			...
			if isSubqueryAlias {
				...
				return plan.NewCachedResults(c.Node), true, nil
			}
		}
	...
	if scope == nil {
		...
		n, err = plan.TransformUpCtx(n, selector, func(c plan.TransformContext) (sql.Node, bool, error) {
			cr, isCR := c.Node.(*plan.CachedResults)
			if isCR {
				return cr.UnaryNode.Child, true, nil
			}
			return c.Node, false, nil
		})
	}
