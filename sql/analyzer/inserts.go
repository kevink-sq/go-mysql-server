...
func resolveInsertRows(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	...
	return plan.TransformUp(n, func(n sql.Node) (sql.Node, bool, error) {
		...
		if insert.IsReplace {
			...
			if !ok {
				return nil, false, plan.ErrReplaceIntoNotSupported.New()
			}
		}
		...
		if len(insert.OnDupExprs) > 0 {
			...
			if !ok {
				return nil, false, plan.ErrOnDuplicateKeyUpdateNotSupported.New()
			}
		}
		...
		return insert.WithSource(project), true, nil
	})
}
