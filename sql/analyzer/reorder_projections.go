...
func reorderProjection(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	...
	return plan.TransformUp(n, func(node sql.Node) (sql.Node, bool, error) {
		...
		return plan.NewProject(projections, child), true, nil
	})
}
...
func addIntermediateProjections(project *plan.Project, projectedAliases map[string]sql.Expression) (neededReorder bool, child sql.Node, err error) {
	...
	child, err = plan.TransformUp(project.Child, func(node sql.Node) (sql.Node, bool, error) {
		...
		switch node := node.(type) {
		case *plan.Filter:
			return plan.NewFilter(node.Expression, child), true, nil
		case *plan.Sort:
			return plan.NewSort(node.SortFields, child), true, nil
		default:
			return nil, false, ErrInvalidNodeType.New("reorderProjection", node)
		}
	})
