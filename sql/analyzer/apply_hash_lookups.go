...
func applyHashLookups(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	return plan.TransformUpWithPrefixSchema(n, nil, func(c plan.TransformContext) (sql.Node, bool, error) {
		...
		if j, ok := c.Node.(plan.JoinNode); ok {
			...
			if j.JoinType() == plan.JoinTypeRight {
				if _, ok := j.Left().(*plan.HashLookup); ok {
					return j.WithMultipassMode(), true, nil
				}
			} else {
		...
		if validCondition {
			...
			return plan.NewHashLookup(cr, secondaryTuple, primaryTuple), true, nil
		}
		return c.Node, false, nil
	})
}
