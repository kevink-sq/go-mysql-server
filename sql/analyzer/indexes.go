...
func convertIsNullForIndexes(ctx *sql.Context, e sql.Expression) sql.Expression {
	expr, _ := expression.TransformUp(e, func(e sql.Expression) (sql.Expression, bool, error) {
		isNull, ok := e.(*expression.IsNull)
		if !ok {
			return e, false, nil
		}
		return expression.NewEquals(isNull.Child, expression.NewLiteral(nil, sql.Null)), true, nil
	})
	return expr
}
