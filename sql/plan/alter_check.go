...
func NewCheckDefinition(ctx *sql.Context, check *sql.CheckConstraint) (*sql.CheckDefinition, error) {
	...
	unqualifiedCols, err := expression.TransformUp(check.Expr, func(e sql.Expression) (sql.Expression, bool, error) {
		gf, ok := e.(*expression.GetField)
		if ok {
			return expression.NewGetField(gf.Index(), gf.Type(), gf.Name(), gf.IsNullable()), true, nil
		}
		return e, false, nil
	})
	if err != nil {
